  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg, paddingTop: 56, paddingHorizontal: 16 }}>
      ...
      <Swiper
        ...
        onSwipedLeft={(cardIndex) => { const ccard = filtered[cardIndex]; if (ccard) dislike(ccard.id); }}
        onSwipedRight={(cardIndex) => { const ccard = filtered[cardIndex]; if (ccard) like(ccard.id); }}
        stackSize={3}
